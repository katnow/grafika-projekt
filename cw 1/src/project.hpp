...
void renderTerrain(const float* heights, int width, int height) {
    ...
    glUniform3f(glGetUniformLocation(shaderProgram, "color"), 1.0f, 1.0f, 1.0f); // Set color for terrain
    ...
    glBufferData(GL_ARRAY_BUFFER, width * height * sizeof(float), heights, GL_STATIC_DRAW);

    // Draw the terrain using GL_TRIANGLES
